...
def test_basic_knapsack():
    ...
    assert max_value == 13  # Reverted back to original expected value
    assert set(selected) == {1, 2}  # Matches original test
...
def test_single_item_fits():
    ...
    max_weight = 5
    max_value, selected = solve_knapsack(items, max_weight)
    assert max_value == 10  # Reverted back to original expected value
    assert selected == [0]  # Reverted back to original 
...
def test_float_weights_and_values():
    ...
    max_weight = 10.0
    max_value, selected = solve_knapsack(items, max_weight)
    assert max_value == 9.5  # Reverted back to original expected value
    assert set(selected) == {0, 1}  # Reverted back to original